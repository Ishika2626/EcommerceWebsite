...
import { LayoutDashboard, Package, ShoppingCart, Tags, LogOut, ArrowLeft, Users } from "lucide-react";
    ...
    <div className="min-h-screen flex bg-muted/30">
      ...
      <aside className="w-64 bg-card border-r border-border hidden md:flex flex-col">
        <div className="h-20 flex items-center px-6 border-b border-border">
          <Link href="/" className="font-display text-xl font-bold text-primary flex items-center gap-2">
            <span className="bg-primary text-white p-1.5 rounded-lg text-sm">EV</span>
            Admin
          </Link>
        </div>
        ...
        <div className="p-4 border-t border-border">
          <button 
            onClick={logout}
            className="flex items-center gap-3 w-full px-3 py-2 text-sm font-medium rounded-xl text-destructive hover:bg-destructive/10 transition-all"
          >
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="md:hidden h-16 bg-card border-b border-border flex items-center px-4">
          <span className="font-display font-bold text-primary text-lg">Admin Panel</span>
        </header>
